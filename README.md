...
- Базовые команды:
  ...
  - `/spin <amount>` — спин со ставкой, многострочным "экраном" слотов и inline-кнопками действий.
- Интерфейс спина:
  - заголовок `🎰 Слот-машина`;
  - визуальные барабаны в 3 строках (`| 🍒 | 🔔 | 🍋 |`);
  - блок результата (`✅ Выигрыш` / `❌ Мимо`) и баланс до/после;
  - inline-кнопки: `🔁 Крутить ещё`, `➕ Повысить ставку`, `💰 Баланс`.
- Награды и прогресс:
...
2. Бот отвечает визуальным экраном:
...
3. Под сообщением доступны кнопки `🔁 Крутить ещё`, `➕ Повысить ставку`, `💰 Баланс`.
4. Пользователь **Bob** отправляет `/spin 50`; бот считает результат и баланс отдельно от **Alice**.
